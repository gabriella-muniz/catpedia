document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(apiUrl, {
    ...
    .catch((error) =>
      console.error("Erro ao buscar informações das raças de gatos:", error)
    );
  ...
  function displayCatBreeds(catBreeds) {
    ...
    createChart(
      "temperament-chart",
      getTopBreeds(catBreeds, "affection_level", 5),
      "Nível de Afeto"
    );
    createChart(
      "social-needs-chart",
      getTopBreeds(catBreeds, "social_needs", 5),
      "Necessidade Social"
    );
    createChart(
      "life-expectancy-chart",
      getTopLifeExpectancyBreeds(catBreeds, 5),
      "Expectativa de Vida Média"
    );
    createChart(
      "affection-level-chart",
      getTopBreeds(catBreeds, "affection_level", 5),
      "Nível de Afeto"
    );
    createChart(
      "energy-level-chart",
      getTopBreeds(catBreeds, "energy_level", 5),
      "Nível de Energia"
    );
  }
  ...
  function getTopBreeds(catBreeds, attribute, count) {
    ...
    return sortedBreeds
      .slice(0, count)
      .map((breed) => ({ name: breed.name, value: breed[attribute] }));
  }
  ...
  function getTopLifeExpectancyBreeds(catBreeds, count) {
    ...
    return sortedBreeds
      ...
      .map((breed) => ({
        name: breed.name,
        value: parseInt(breed.life_span.split(" ")[0]),
      }));
  }

  function buscar() {
    const searchTerm = document.getElementById("search").value.toLowerCase();
    const raceFilter = window.catBreeds.filter((cat) =>
        cat.name.toLowerCase().includes(searchTerm)
    );
    displayCatBreeds(raceFilter);
}

function reagirMudanca() {
    buscar(); // função de busca quando houver uma mudança no campo de busca
}

document.getElementById("search").addEventListener("input", reagirMudanca);
